...
export default function NavigationCard({ href, title, description, onClick, className = "" }: NavigationCardProps) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className={`group relative backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl px-4 py-4 shadow-xl hover:bg-white/15 transition-all duration-300 hover:scale-105 hover:shadow-2xl overflow-hidden flex-1 ${className}`}
    >
